...
The above results was acheived when we set the seed to 1. As we can see, the $R^2= 58.1058840459775$, this is not the best result that we can achieve (maybe we can achieve $R^2\geq 0.6$). Therefore, we came up with a simulation that can give us the best result possible. Since we need a seed to keep the results consistent and reproducible, we looped through numbers 1 through 100 to set as possible seeds. Then we chose a range for M, in this case $M\in [1,4]$. 
