...
We needed a dataset that was known to perform well and has been tested with different machine learning algorithms extensively. After looking through the machine learning repository, we chose the Wine Quality dataset as it was one of the more popular datasets for regression.

This dataset Wine Data has two datasets, white wine and red wine, we comebined the datasets and used that merged dataset to perform regression. The predictors are the physicochemical tests of the wine and based on those predictors we will attempt to preform regression.
...
Similarily, we needed a dataset that is binary (2 classes), so its simple and it will classify well. We chose the Wisconsin Breast Cancer data from the Machine Learning Repository. Its a dataset that has 699 observations and 10 features. The features compose of things such as uniformity of cell size, the uniformity of the cell shape, cell size etc. And as said before, it has two classes class 2 for benign, 4 for malignant (but for coding purposes we changed it to 0 and 1 for benign and malignant respectively).

For ease of reading in the data, this data has been preprocessed already regression. This data is is preprocessed already so we have to read in the data only.
