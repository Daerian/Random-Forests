...
From here on out, we started to work on other functions. We needed a way to actually create the forest. So, we first began by creating a functiont that took as input the number of boostraps, the number of features for each tree, and the data the trees would be based on. We took these inputs, and created as many bootstrap samples as the input specified, creating a tree using the previous function using this bootstrap as the data instead! So far so good. We had a way to itteratively create trees for each boot strap sample. There was however a key problem tht had arrised: How do we store these trees? How do we retreive which observations were affected by the tree? Which ones could we test on? As it turns out, R doesn't allow us to create data-types with the same efficiency as other platforms, so we had to improvise. Our solution was to create empty lists for everything and then append to them. Each tree return would now consist of the tree it self, the set of all observations in the original training data that the tree did NOT use to train (making cross validation ... valid on these sets) and the list of the variables that it has found to be the most important in prediction (out of the ones chosen for it's features only.)
These lists returned by the tree function would find themselves added to a nother list inside the froest function which stores all the trees returned for each bootstrap. This is then made additionally nested, since the tree itself is also a list! Each part of the tree (such as the plits, it's important variables, the methods it has, function calls, etc) is also stored in a list or a series of nested lists. As you can imagine this results to objects of substantial order. One of the challenges of creating this script was tracking the location of things we needed in this large cluster of topography. Fortunately, we found a way to work with this chaos, and retreived all the things we needed.
...
From here we had to come up with and create classification and regressive comparisons and ways to evlauate the performance of the random forest ont he test data.
...
Under classification we have Loss function, which calculates false possitives. For regression we calculate a MSE and a coefficient of determination using the formulas involving the error terms. This allows us to account for all possibile conditional distributions of the multivariable regressions, since we use only the final residual, which is the leftover from the total minus the sum of all regressive sum of squares.
...
In addition we came to the realization that the formula for MSE and R2 we were using relied on Regression. However as we were working with multiple predictors, we had no choice but to redo the function to use the error term instead fo regular regression. This is due tot he fact that with multiplepredictors the MSE is unique to each of them, conditional on the rest. So, to get true error we have to use the error term which is residual from the final conditional subtracted from the total. As it turns out the math her eis equivalent, however we have a value that is present for all datasets.
...
Finally we implemented the control mechanic. We can now control the number of splits, the number of cross validations for each tree, the minimum increase of R2 per split, etc. This function also increased our own values by a significant amount. The issue is that majority of these are hyper-parameters that we have to come up with outselves.
...
The above results was acheived when we set the \texttt{seed} to 1. As we can see, the $R^2=0.0953803998083936$, let's see how high the $R^2$ can go. Therefore, we came up with a simulation that can give us the best result possible. Since we need a seed to keep the results consistent and reproducible, we looped through numbers 1 through 100 to set as possible seeds. First, we split the training set and testing set 50\% and 50\% respectively. this give us around the same number as when we set the training and testing set to 75\% and 25\% respectively. We decided to report the bigger number (obviously) which is $R^2=  0.277918789503239$ and $MSE= 0.529555537034112$ when $B=1500$ (number of trees), \texttt{seed}$=64$ and $M=5$ (number of variables used per tree). This code can be found in \texttt{experiment.R} (it takes at least 10 minutes to run depending on the parameter). 
```{r, message=FALSE}
set.seed(64)
